<?php
    ...
    function send_password_reset($get_name,$get_email,$token)
    {
        ...
        $email_template="
                    ...
                    <br/>
                    <a href='http://localhost/PHP/project/project1/password_change.php?token=$token&email=$get_email'>Click Me</a>";
        ...
        if(!empty($token))
        {
            if(!empty($email)&& !empty($npwd)&&!empty($cpwd))
            {
                ...
                if($check_token_count>0)
                {
                    if($npwd==$cpwd)
                    {
                        $update_password="UPDATE student SET password='$npwd' WHERE verify_token='$token' LIMIT 1";
